...
const Home = ({ navigation }: any) => {
    const renderUiWidget = () => {
        const renderIconWithText = (iconSource: string, text: string, color: string) => (
            <TouchableNativeFeedback onPress={() => navigation.navigate(ScreenConstant.EDIT_PROFILE)}>
                <View style={styles.otherIconsContainer}>
                    <Icon source={iconSource} size={36} color={color} />
        ...
        return (
            <View style={styles.walletContainer}>
                {renderIconWithText('account', 'Account', 'green')}
                {renderIconWithText('bell', 'Thông báo', 'gray')}
                {renderIconWithText('cog', 'Cài đặt', 'red')}
            </View>
        );
    };
